...
public:
	...
	void	SetDirID(Bit32u entry)			{ sSave(sDTA, dirID, entry); };
	Bit32u	GetDirID(void)					{ return (Bit32u)sGet(sDTA, dirID); };
private:
	#ifdef _MSC_VER
	#pragma pack(1)
	#endif
	struct sDTA {
		...
		Bit32u dirID;						// custom: dir-search ID for multiple searches at the same time
		Bit8u fill[4];
		Bit8u attr;
		Bit16u time;
		Bit16u date;
