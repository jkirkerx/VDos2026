...
class Disk_File : public DOS_File
	{
public:
	Disk_File(const char* name, int fd);
...
bool DOS_Drive::AllocationInfo(Bit16u *bytes_sector, Bit8u *sectors_cluster, Bit16u *total_clusters, Bit16u *free_clusters)
	{
...
bool DOS_Drive::FileCreate(DOS_File * * file, char * name, Bit16u attr/*attributes*/)
	{
...
bool DOS_Drive::FindFirst(char* _dir, DOS_DTA & dta, bool fcb_findfirst)
	{
	if (!TestDir(_dir))
		{
...
//		FindClose((HANDLE)dirID);													// Jos: I would like to do this (saving resouces?)
		...
		}																			// So the DirID gets an old value. DOS_Drive::FindNext() uses this for FindNextFile() and vDos is terminated by Wijdwos!!!
...
bool DOS_Drive::FindNext(DOS_DTA & dta)
	{
	Bit8u srch_attr;
	char srch_pattern[DOS_NAMELENGTH_ASCII];
	...
	do
		{
		if (search_data.cFileName[0] == '.' && !strncmp(basedir, srch_dir, strrchr(srch_dir, '\\')-srch_dir))
			continue;																// Don't show . and .. folders in vDos root drives
		bool dosName = false;
		if (!isDosName(search_data.cFileName))										// If it's not a DOS 8.3 name
			if (strcmp(srch_pattern, "        .   ") && !strchr(srch_pattern, '?') && !strchr(srch_pattern, '*'))
				dosName = true;														// We allow it only if Find was called to test the existance of a specific SFN name (no wildcards)
			else
				continue;
...
bool Disk_File::Read(Bit8u* data, Bit16u* size)
	...
	for (int tries = 2; tries; tries--)												// Try twice
		if ((bytesRead = _read(fd, data, *size)) != -1)
			{
			*size = bytesRead;
			return true;
			}
...
bool Disk_File::Write(Bit8u* data, Bit16u* size)
	...
	int bytesWritten;
	for (int tries = 2; tries; tries--)												// Try twice
		if ((bytesWritten = _write(fd, data, *size)) != -1)
			{
			*size = bytesWritten;
			return true;
			}
...
Disk_File::Disk_File(const char* _name, int _fd)
	{
	fd = _fd;
