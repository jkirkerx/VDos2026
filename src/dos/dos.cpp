...
static Bitu DOS_21Handler(void)
	...
	case 0x4b:																		// EXEC Load and/or execute program
		if (reg_al > 3)
			...
			else if (setResult)														// NB DON"T if succeeded executing (stack is of new program)
				CALLBACK_SCF(false);
			}
		break;
	...
	case 0x4e:																		// FINDFIRST Find first matching file
		...
		if (!stricmp("C:\\COMMAND.COM", name1) || DOS_FindFirst(name1, reg_cx))		// What about DTA with C:\COMMAND.COM?
			{																		// Go into the trouble for DOS 2, 0x11?
