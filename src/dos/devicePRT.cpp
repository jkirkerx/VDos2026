...
static void DumpTcpRaw(const char* filename, const std::string& data)
	{
...
void device_PRT::CommitData()
	...
	if (tcpConfigured)
		...
		std::string detail;
		if (printDebug)
			DumpTcpRaw(tcpRaw, rawdata);
		bool sent = TcpPrinter_Send(tcpHost, tcpPort, rawdata, detail);
		if (printDebug)
			LogTcpPrint(tcpLog, GetName(), tcpHost.c_str(), tcpPort, rawdata.size(), sent, detail, tcpRaw);
		if (!sent)
