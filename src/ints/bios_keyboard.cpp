...
bool BIOS_AddKeyToBuffer(Bit16u code)
	...
	if (ttail >= Mem_aLodsw(BIOS_KEYBOARD_BUFFER_END))
		ttail = Mem_Lodsw(BIOS_KEYBOARD_BUFFER_START);

	if (ttail == Mem_Lodsw(BIOS_KEYBOARD_BUFFER_HEAD))								// Check for buffer Full
		return false;
	Mem_Stosw(0x40, tail, code);
	Mem_Stosw(BIOS_KEYBOARD_BUFFER_TAIL, ttail);
	return true;
	}
...
static void add_key(Bit16u code)
 ...
 bool BIOS_GetKey(Bit16u &code)
	...
	Mem_Stosw(BIOS_KEYBOARD_BUFFER_HEAD, thead);
	code = Mem_Lodsw(0x40, head);
	pasteKey();
	return true;
	}
...
void UpdateKBflags()																// At least L.Shift + R.Shift are not captured by BIOS_addkey, so seperate
	...
	flags1 = kbState[VK_RSHIFT]>>7;													// Flags are mostly masked by shifting
...
static Bitu INT16_Handler(void)
	...
	case 0x02:																		// Get shift flags
		reg_al = Mem_Lodsb(BIOS_KEYBOARD_FLAGS1);
		break;
	...
	case 0x12:																		// Get extended shift states
		reg_al = Mem_Lodsb(BIOS_KEYBOARD_FLAGS1);
		reg_ah = Mem_Lodsb(BIOS_KEYBOARD_FLAGS2);
		break;
		};
	return CBRET_NONE;
	}
