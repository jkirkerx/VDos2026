...
static bool BuildPrintDebugPath(char* filename, size_t filenameSize, const char* name)
	{
	...
	strcat_s(filename, filenameSize, "\\");
	strcat_s(filename, filenameSize, name);
	return true;
	}

static void WritePrintDebugStartupMarker(const char* cmdLine)
	{
	char filename[MAX_PATH];
	if (!BuildPrintDebugPath(filename, sizeof(filename), "vDosPrintDebug-startup.txt"))
		return;
	...
	fprintf(fh, "%s -printDebug enabled cmdLine=%s exe=%s\n", stamp, cmdLine ? cmdLine : "", _pgmptr);
	fclose(fh);
	}
...
static void ParseStartupOptions(char* cmdLine)
	{
	char buffer[1024];
	...
	while (token)
		{
		if (!stricmp(token, "/log") || !stricmp(token, "-log"))
			logMSG = fopen("vDos.log", "w");
		else if (!stricmp(token, "-printDebug") || !stricmp(token, "/printDebug"))
			{
			printDebug = true;
			WritePrintDebugStartupMarker(cmdLine);
			}
