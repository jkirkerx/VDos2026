...
static Bit8u exe_block[]={
	...
	0x00, 0x00,																		// Callback ID at position 12
...
Program::Program()
	...
	CommandTail tail;
	Mem_CopyFrom(SegOff2Ptr(dos.psp(), 128), &tail, 128);
...
bool Program::SetEnv(const char* entry, const char* new_string)
	...
	do
		{
		Mem_StrnCopyFrom(env_string, env_read, 256);
		if (!env_string[0])
			break;
		...
		Mem_CopyTo(env_write, env_string, (Bitu)(strlen(env_string)+1));
...
// TODO Maybe save the program name sometime. not really needed though
	...
	Mem_Stosd(env_write, 0);														// Clear out the final piece of the environment
	return true;
	}
...
void PROGRAMS_Init()
	...
	CALLBACK_Setup(cbID, &PROGRAMS_Handler, CB_RETF);								// Internal program
	*(Bit16u*)(&exe_block[12]) = cbID;
	}
